...
```
> The dataset contains a lot of different responses and datatypes. Further, tere are also various NA values that we will need to clean from the data in a next step. Below we will clean the data in a first step.
...
> Interestingly Nonprofits are the second most frequent industry of the survey, an industry usually less mentioned.
...
> The vast majority of the dataset is white. 
...
```
> The survey asked mostly fairly experienced people.
...
> Vast majority of respondents are women. This might explain the high count of people working in nonprofits.
...
> Men and interestingly "don't know or NA" have the highest annual salary.
...
Based on the exploratory data analysis above, we observe a clear relationship between annual salary and field experience, race, gender, education level, industry and state. We will analyze the possibility of including these variables in our model. Now, since we observe that all these explanatory variables are categorical, we try merging some of the categories of these variables, based on the count of people and mean salary in each category.
...
We will proceed with VIF calculation and Residual Analysis for Model6 (with field_exp_grouped, race_cleaned, gender, edu_level_grouped, industry_grouped and state_grouped as explanatory variables). We observe that the regression model is significant at 95% confidence level as the overall p-value is less than 0.05. Most of the variables are significant and we have the highest Adjusted R^2 (44%) for this model out of all the iterations. This means that this model is able to explain 44% of the variability in log of annual salary.
