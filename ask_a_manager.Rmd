...
us_data %>% 
 ...
 top_n(15) 
  ...
  # 
  # #plot
  # ggplot(aes(x=fct_reorder(city,mean_salary, .desc=TRUE),y= mean_salary))+
  # geom_col(fill= "blue")+
...
```{r, boston health, san franscisco tech}
...
#ADD COMMENTS ABOUT SF AND TECH
```
...
```{r, t test between salaries for defirrent racess}
us_data%>%
  group_by(race_cleaned) %>% 
  summarise(mean_salary = mean(annual_salary),
            median_salary = median(annual_salary),
...
HYPOTHEISS THEST
asians in tech LATER
SLAARY IN WOMEN AND MEN IN TECH DONE
# RACE SALARY IS TI DIFFERENT (WHITE VS OTHER) DOOOONE
EDUCATIONS VS RACE
