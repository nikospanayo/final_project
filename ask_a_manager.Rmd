...
## 1 Ask a manager: Data on annual salary from a survey
...
The dataset obtained from the survey  contained a large number of respondents that were predominantly white females with high education levels.

Due to the limited observations for countries other that the US we decided to focus our entire analysis only on US-Salary (respondents from the US). In addition, we eliminated some of the columns that were not directly related with annual salary and/or contained too many individual answers (i.e. job title) unable to conclude something from them. After building a clean and workable dataset, the exploratory data analysis was performed and visualized to get a better understanding of the survey respondents and the data in general. 
We build the following `hypothesis test`:

- Hypothesis 1: Whether the average annual salary for tech industry is difference between man and woman
- Hypothesis 2:Whether people in San Francisco tend to work in tech industry compared to other cities in America
- Hypothesis 3: Whether Asian salary is higher than other races.
- Hypothesis 4: Whether Asian tend to work in tech industries than other fields


Thereafter, we built a `regression model` through iteration and using a test train model. The final model includes the independent variables: X, Y, Z which has a R-squared value of XXX. The final regression model is used to XXXX
...
```
> The dataset contains a lot of different responses and datatypes. Further, there are also various NA values that we will need to clean from the data in a next step. Below we will clean the data in a first step.

- Rename columns
- Delete columns (i.e. time stamp, other monetary comp)
- Changed character data into categorical (i.e. Age, overall experience, education level)
- Clean country name using countrycode()
- Delete industries with <100 respondants
- Delete race with <100 respondants
- Regroup race into only 5 
- Dropping NA values
...
> To get a more comprehensiv picture we will only focus on respondents with a salary between 10'000 and 1'000'000 USD.
...
> To get a sense of the survey respondents the following graphs explore the respondents origin, their gender, race etc.
...
# 2.2.1 Analysis on US-Salary

> In the following exploratory data analysis is conducted on the different variables to thereafter continue with hypothesis testing and regression models.
...
```
> Vast majority of survey respondents are between 25-44 indicating that the mean salary is probably higher than the mean salary of the entire US population.
...
> Higer propotion of respondents work in computing or tech industry. Interestingly Nonprofits are the second most frequent industry of the survey, an industry usually less mentioned.

```{r salary in different industry}
...
us_data %>% 
  ...
  geom_boxplot() +
  #add labels
  labs(title = 'Boxplot of annual vs industry',
        x = 'Annual Income',
        y = 'Industry') +
  theme_bw()+
  guides(fill= FALSE)
...
us_data %>% 
  ...
  NULL
```
> The cities most often mentioned are all large cities in the USA. Geographically, however, they are spread across the entire country.
...
> The survey also display a very high education level in general.
...
```
> Years of professional experience is close to normal distribution. The largest propotion is people with 11-20years of experience.
...
#plot barchart  
 ...
 geom_text(aes(label=round(mean_salary,0)),size=3,vjust=-0.25)+
  ...
  theme_bw()
```
> Average salary of man is higher than woman an non-binary respondents.
...
us_data %>% 
  ...
  guides(fill=FALSE)
```
> To futher investgate why salary of man is higher than woman, we make a graph of their industry. We filter induatries than have more than 500 respondents and surprising find that all man work in computing or tech industry.
...
Following these observations,we want to compare whether the average annual salary for tech industry is difference between man and woman.
...
> We use confident interval and t test to show the difference.The confident interval of man is [142345,149903] which is not overlapped with woman [116518,121502] and p value in t test is <2e-16. Therefore we reject the null hypothesis and conclude there is a significant difference in annual salary between man and woman who work in tech industry.
...
> 48% San Francisco vs. 16% in other US cities. Proportion of tech-employees in San Francisco is significantly higher than in other US cities as P-value < 2e-16. 
...
> By comparing the confidence interval of annual income between each race, we see there is no overlap between Asian and other races. And t test between Asian and White has p-value <2e-16. Therefore we conclude annual salary of Asian is significantly higher than other races.
...
us_data %>% 
  group_by(city) %>% 
  filter(city %in% c("San Francisco", "Seattle","New York","Washington","New York City")) %>% 
  mutate(
...
> The porption of Asian in Tech (9.39%) is significantly different from Asian in other industries (5.38%) as P-value < 2e-16. We can conclude Asain tend to work in tech industries.
...
```{r, distribution_of_salary_and_log_salary, fig.with = 12, fig.height=4}
